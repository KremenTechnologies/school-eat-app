...
- Telegram шле сервер (`httpx`, HTML, час — Europe/Kyiv). Токен у `settings`, не в браузері. Повідомлення:
  - **Відвідування** — автоматично при кожному `PUT /api/attendance/{date}/{class}` (тобто щоразу, як клас зберігає дані). Клас, дата (укр. + день тижня), присутні/відсутні, розбивка причин + прізвища, хто і о котрій вніс.
  - **Заявки на харчування** — вручну, кнопка «Надіслати зведення в Telegram» на вкладці «Заявки» (`POST /api/attendance/summary?date=`). Моноширинна таблиця клас × присутні × сніданки × обіди + «Разом».
  - **Тест** — кнопка у «Налаштуваннях» (`POST /api/telegram/test`).
  - Усе в один чат (`chat_id` з `settings`). Не налаштовано → save проходить, у відповіді `telegram.error`.
